...
1. Pre-flight — inspect repo state and rule out interference before touching anything:
   ...
   - **Concurrency check: `git reflog -8` plus a status snapshot.** Record `git status --porcelain=v1 --untracked-files=all` at pre-flight, then compare it again before verification, before commit, and before runtime restart. Reflog catches checkout/merge interference, while the status comparison catches another agent that only adds or edits files without moving HEAD. If either signal shows concurrent work, do NOT verify or release from the main checkout — a concurrent agent can switch HEAD mid-run or mutate the tree being tested. Instead: create a repo-external release worktree from the intended commit, run ALL remaining release steps there, and skip the shared runtime restart when new unrelated paths appeared during the run. Remove the release worktree after publishing.
2. Audit the pending diff for release blockers before any commit:
   ...
   - if the checkout mixes a release-ready slice with unfinished user/agent WIP, preserve that WIP in place: create a repo-external release worktree, apply only a path-limited patch for the audited slice, and run verification there. When the highest published tag contains a release-only version/skill commit that the target branch does not yet have, base the isolated release branch on that published tag's peeled commit instead of the stale package version; otherwise use the intended target-branch base commit. Commit and push from the isolated release branch when mirroring back would disturb the excluded WIP; do not stash, revert, or accidentally commit the excluded WIP.
...
3. Verify — a release warrants the release posture from `../chill-vibe-full-regression/SKILL.md`:
   ...
   - if a narrow failure looks pre-existing, prove that instead of guessing: reproduce the same test against the exact release base commit in a temporary detached comparison worktree, then remove it. Use the highest published tag's peeled commit when the release candidate is based on that tag; use `origin/main` only when it is the actual base. Only classify the failure as baseline noise when the release candidate's affected/targeted gates pass and the baseline shows the same failure.
   - if the Node test stage prints its final assertions but the wrapper stays alive with no output or CPU because of leaked handles, terminate only that verified wrapper process tree and rerun the same suite with `node --import tsx --test --test-force-exit tests/index.test.ts` to obtain a trustworthy pass/fail summary. If the force-exit run still lacks a summary, search its log for completed failing cases and inspect descendants before stopping anything: let active Electron/Git children finish, then rerun every reported failure narrowly (and baseline-compare it when needed). Do not treat assertion output without the runner summary as a green unit gate. If both the original and force-exit runs eventually produce summaries, investigate the union of failures from both runs; do not let one timing profile erase failures reported by the other.
...
- A release worktree install can fail with `EPERM` when an active Electron runtime locks the shared pnpm Electron package, or finish green while reporting `Ignored build scripts: electron` and leave `electron.exe` absent. Never stop a user-facing packaged app just to unblock verification: reuse an already verified dependency tree through a safe worktree junction or wait for the lock; when only Electron's postinstall was skipped, run `node node_modules/electron/install.js` and verify `node_modules/electron/dist/electron.exe` before Electron gates.
