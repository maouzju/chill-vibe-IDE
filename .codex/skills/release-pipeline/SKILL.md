...
2. Audit the pending diff for release blockers before any commit:
   ...
   - **a feature that adds one UI element breaks Playwright in two ways that no Node or quality gate can see**: hard-coded `toHaveCount(n)` assertions, and every screenshot whose frame contains the new element. v0.19.0 inherited 13 stale counts and 26 stale snapshots from an already-merged feature branch, all invisible until the 4-minute Playwright stage 20 minutes into the gate. If the candidate adds a card type, a menu row, or a launcher tile, grep the specs for counts of that surface *before* launching the gate.
...
4. Verify the final versioned tree — a release warrants the release posture from `../chill-vibe-full-regression/SKILL.md`:
   ...
   - when a short `spawnSync`/`Promise.race` guard times out only inside the full manifest, rerun that file alone and compare the normal duration with the guard. If the isolated path is comfortably fast and the candidate did not change it, harden the harness margin (roughly 5× normal cold-start cost) and rerun the new exact fingerprint instead of waiving the red gate or blaming product code.
...
- PowerShell array-to-argument joining breaks the manual upload path twice, both times before any bytes move. Uploading a zip whose path contains spaces: avoid `Start-Process -ArgumentList @(..., $zip, ...)`, which joins the array into one string and lets `gh release upload` split `Chill Vibe-...zip` at the space — use the call operator, `& gh release upload v<tag> $zip --clobber`. Generating temporary curl config lines: wrap concatenated strings in parentheses inside the array, e.g. `('output = "' + $path + '"')`, or PowerShell spreads the value across multiple lines and curl fails to parse it.
