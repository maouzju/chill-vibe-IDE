...
3. Verify — a release warrants the release posture from `../chill-vibe-full-regression/SKILL.md`:
   ...
   - if fixed port `5173` is owned by an unrelated checkout, do not stop or reuse that process. Record the Playwright gate as port-blocked (many specs hard-code `5173`), then still prove the Electron runtime on a free port: start repo Vite hidden with `pnpm exec vite --host 127.0.0.1 --port <free-port> --strictPort`, health-check it, set `VITE_DEV_SERVER_URL=http://127.0.0.1:<free-port>` plus `CHILL_VIBE_HEADLESS_RUNTIME_TESTS=1`, and run the same Electron runtime test files directly with `node --import tsx --test ...`. Do not run the stock `test:electron` while an unrelated service owns `5173`; its harness treats any listener as the intended renderer and can validate the wrong app.
