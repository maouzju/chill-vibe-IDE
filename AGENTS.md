...
| 60 | Editing Electron desktop backend files like `server/providers.ts` or `electron/backend.ts` while relying on renderer HMR can leave the visible app on stale runtime logic | Vite refreshes the UI, but the active Electron main/backend process keeps the old handlers until `pnpm dev:restart` (or an equivalent full Electron restart) is run. |
| 61 | Codex app-server `thread/resume` can return an idle thread for an interrupted card, and a follow-up `turn/start` with `input: []` then fails with `input must not be empty` | Resume recovery looks like a silent no-op unless the idle resumed thread is nudged with a blank text input item, not an empty input array. |
| 62 | `pnpm test:quality` can be red from pre-existing unused variables in `src/components/GitFullDialog.tsx` even when the touched column-resize change is fine | Use file-scoped lint plus the narrow unit/Playwright checks for the changed surface, and treat the broad quality failure as unrelated branch debt unless you are also fixing that dialog. |
