...
- `workspacePath` 只是 CLI 的 cwd，不是文件系统权限边界。普通 Codex 会话仍可保持既有 `danger-full-access + approvalPolicy=never`，Claude 会话仍可使用 `bypassPermissions`，但 IDE 默认向两种 CLI 注入共享判定逻辑的 `PreToolUse` 高风险删除防护，并在设置中允许用户明确关闭。Codex Hook 未被当前 CLI 发现、信任或启用时必须失败关闭，不能静默裸跑；Claude 通过本次运行的 session-level `--settings` 注入 `Bash` Hook，不修改用户设置文件。
...
- Agent destructive-command protection is structural rather than prompt-only: IDE-owned `PreToolUse` hooks for Codex and Claude CLI share one guard that canonicalizes deletion targets and blocks user-home/workspace-root ancestors, paths outside the workspace, drive roots, `.git`, Codex/Chill Vibe data, unresolved/runtime-expanded variables, recursive wildcards, relative recursive-delete targets, PowerShell `$home` assignment collisions, `TemporaryDirectory` + bind-mount cleanup hazards, and broad destructive Git operations. Relative recursive deletes fail closed because the hook payload omits a shell tool's separate `workdir`; explicit absolute workspace-child targets remain allowed. All Codex app-server processes use one stable session-flags hook command/hash and pass protected roots through process-local environment variables, because Codex uses the same positional trust key for every session-flags hook. Trust is verified through `hooks/list` + `config/batchWrite`; never use `--dangerously-bypass-hook-trust`, which would also run unrelated untrusted hooks. Claude receives the same launcher through session-level `--settings` as a `PreToolUse` / `Bash` hook; exit code `2` blocks the call, user hooks remain additive, and the keepalive signature includes the safety configuration so toggles recycle the pooled process.
