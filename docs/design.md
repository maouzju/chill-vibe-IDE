...
- Codex 恢复旧会话时，如果 CLI 报告 rollout 为空（包括新版 thread store 的 `failed to read session metadata ... rollout ... is empty` 文案），应视为原生会话已损坏，自动新建线程并继续发送当前需求，而不是把内部文件错误直接显示给用户。
- `size` 表示卡片的最小高度
...
- Agent safety protection is structural rather than prompt-only: IDE-owned `PreToolUse` hooks for Codex and Claude CLI share one guard. Its independent high-risk deletion layer canonicalizes deletion targets and blocks user-home/workspace-root ancestors, paths outside the workspace, drive roots, `.git`, Codex/Chill Vibe data, unresolved/runtime-expanded variables, recursive wildcards, relative recursive-delete targets, PowerShell `$home` assignment collisions, `TemporaryDirectory` + bind-mount cleanup hazards, and broad destructive Git operations. Its optional workspace-write layer canonicalizes direct `Edit` / `Write` / `NotebookEdit` targets (including symlink/junction ancestors) and common shell write destinations before allowing them. All Codex app-server processes use one stable session-flags Hook command/hash and pass protected roots plus feature flags through process-local environment variables, because Codex uses the same positional trust key for every session-flags Hook. Trust is verified through `hooks/list` + `config/batchWrite`; never use `--dangerously-bypass-hook-trust`. Claude receives the same launcher through session-level `--settings`; exit code `2` blocks the call, user hooks remain additive, and the keepalive signature includes the safety configuration so toggles recycle the pooled process.
- 当当前 provider 的本地 CLI 不可用时，聊天发送仍然要进入应用层校验，并在卡片内追加“本地 CLI 不可用”的系统提示；不能只禁用发送按钮让用户反复点击却没有反馈。
- Provider 状态尚未加载、状态查询失败或刷新中的“未知”不能等同于 CLI 不可用；只有状态明确返回 `available: false` 时才能拦截发送。未知状态应继续尝试真实 CLI 请求，并保留上一份已知状态。
- Claude 流式输出的健壮性（`server/claude-structured-output.ts` + `server/providers.ts` + `server/provider-stream-recovery.ts`）：
  ...
  - Markdown strong-span repair must preserve delimiter context across inline-code boundaries. A balanced fragment such as `**label \`code\`**` must remain unchanged instead of being misclassified as truncated and receiving a visible extra `**` at the end of the reply.
...
- 关闭有路径的工作区时，还要把最后的标签页、pane 分栏、激活页和可持久化卡片状态写入独立
  `closed-workspaces/` sidecar；主 `state.json` 不携带整列快照。
- 用户在未使用的新列中重新选择同一路径时，自动恢复最近关闭的标签布局；不得覆盖已有内容的列，
  也不得在同一路径仍有其他打开列时复制旧快照。
- 精确快照恢复后，关闭动作生成的同批历史副本从索引中移除；旧版本没有 sidecar 时，仅从最新
  紧邻归档批次尽力恢复聊天标签。
- 跨版本后如果 `state.json` 的轻量历史索引与 sidecar 归档不一致，内部历史菜单会按需启动
  故障安全的后台目录维护。维护不阻塞启动，按文件数、总字节、单文件大小和耗时硬切片；
  损坏或过大的归档宁可跳过也不在主进程中无界解析。sidecar 正文始终不被自动删除或覆盖。

### 6.5 关闭桌面窗口

- Electron 桌面版在“设置 → 实用”提供“关闭后最小化到任务栏”选项，默认关闭，旧状态缺字段时也保持关闭。
- 开启后，用户点击窗口关闭按钮只会把窗口最小化到任务栏，Electron 与后台 Agent 继续运行；从任务栏恢复后保留原状态。
- 设置变更必须立即同步给 Electron 主进程，不依赖延迟持久化或重启。
- 明确退出、安装更新和已经进入刷盘退出流程的关闭动作不得被该选项拦截；关闭选项时继续沿用现有的刷盘后退出流程。
