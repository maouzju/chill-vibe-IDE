...
3. Any `subAgentActivity` item registers or updates its target child and canonical path. This also covers nested descendants.
   ...
   Exception: when the target is the root thread (`agentThreadId === rootThreadId` or `agentPath === '/root'`) — which is how codex 0.144.1 reports a child sending a message back to its parent, from the child's point of view — the root is never registered as a child. The interaction is attributed to the sending child's preview instead (`Contacted /root`). Registering the root would make every later root notification look like tracked-child traffic and swallow the parent's commands, deltas, and `turn/completed` (AGENTS.md pitfall #355).
   Generalisation (2026-09-12, codex 0.153.4, AGENTS.md pitfall #370): the same report-back shape appears one level down — a grandchild's `send_message` to its parent is pushed from the grandchild's perspective as `subAgentActivity{agentThreadId: <parent>, agentPath: <parent path>, kind: 'interacted'}`. An `interacted` whose **sender is a descendant of the target** (parentThreadId chain, falling back to canonical-path prefix when the nested `thread/started` never arrived) is attributed to the sender's preview and never changes the target's status; an idle parent that merely receives mail does not start a new turn, so nothing would ever settle a resurrected `running`. Downward contact (parent → child `followup_task`/`send_message`) still marks the child running because it does start a child turn.
...
9. When the run ends by any path (`finishWithDone`, `finishWithError`, including the hard cap and the native-rollout completion fallback), the tracker settles every still-running child (`completed` on done, `interrupted` on error) and pushes that snapshot **before** the terminal event. The renderer's docked panel renders the latest snapshot's running entries only, so without this push an idle card keeps showing "运行中" forever. On the normal path all children have already settled and the push is a no-op.
