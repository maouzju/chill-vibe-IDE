...
> **连续输入卡顿跟进（2026-08-07）**：现场反馈表明，多 agent 同时 streaming 时，持续中文输入仍会被流式 reducer commit 周期性打断。原交互保护仅 120ms、最长延后 300ms；连续输入超过该窗口后，主 App reducer 仍会每约 300ms 提交一次整列更新。现将保护窗口调整为 **250ms，单次最长延后 1s**：输入持续期间让出更多主线程帧预算，停止输入后首个安静窗口立即补刷，且 1s 硬上限避免流式输出永久饥饿。对应守护测试已更新。
>
