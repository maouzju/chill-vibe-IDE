...
- 自动解决合并冲突不设置 Git 同步层的耗时上限，不因任务运行时间长而调用 `stopChat`。
- 等待 AI 流实际完成或报错；完成后重新检查仓库，仍有冲突则报错，无冲突才继续推送。
- 不改变 Provider 本身的连接异常处理、Git 分析超时、关闭面板或重试语义。
- 本次为局部修复，沿用现有规格，不新建 SPEC；守卫测试以虚拟时钟推进 24 小时，确认任务仍等待，完成后才推送，并覆盖真实错误不推送。
