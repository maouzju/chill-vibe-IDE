...
| Opus 4.8 | `claude-opus-4-8` | `opus-4.8`, `claude-opus-4-8`（裸 `opus` 移交 Opus 5） |
...
无迁移逻辑：`claude-opus-4-8` 仍是合法可用模型，`normalizeStoredModel` 不得改写它
（Pitfall #119 —— 把仍可用的模型当成 legacy 默认会覆盖用户在 设置 → 模型 里的显式输入）。
...
| 选择器顺序 | 同上 | claude 选项列表含 `claude-opus-5`，位于 fable 与 opus-4-8 之间 |
| 别名 | 同上 | `opus` / `opus 5` / `claude-opus-5` → Opus 5；`opus 4.8` / `claude-opus-4-8` → Opus 4.8 |
