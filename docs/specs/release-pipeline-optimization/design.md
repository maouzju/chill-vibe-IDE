...
Force-updating refs is not the same as server-side object destruction: GitHub may continue serving an unreachable commit/blob by SHA from its object database or cached views. After a rewrite, verify the public heads/tags with the GitHub API, record any old object ids that remain addressable, and open a GitHub Support purge request. Closed or third-party pull refs are documented as permission boundaries; they must never be presented as cleaned merely because the default branch is clean.
