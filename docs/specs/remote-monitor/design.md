...
- `RemoteMonitorCardSnapshot.modelOptions` 从 `{ model, label }[]` 扩为 `{ provider, model, label }[]`。快照直接遍历共享 `MODEL_OPTIONS`，按 `isModelPickerOptionVisible()` 过滤工具模型，不再按卡片当前 provider 截断列表。
- 手机端 `<option>` 同时保存 provider 与 model；change 事件从当前选项读取两者并发送 `set-card-model`，从 Codex 切 Claude（或反向）时复用现有 renderer handler 完成 session 作废与状态更新。
- `backBtn` 移入已有 `position: sticky` 的全局 `<header>`，详情页显示、列表页隐藏。打开详情时页头标题改为卡片名，返回列表时恢复“Chill Vibe 监工”，保证长会话中随时能返回。
- 根文档使用 `min-height: 100%` 而不是固定 `height: 100%`；固定高度会把 sticky 元素的包含块限制为一个视口，长会话滚过首屏后页头仍会被带走。
