...
## V2.4 — 手机端模型选择与吸顶导航（2026-07-15 追加）

- R19: 手机端详情页的模型选择器必须像电脑端一样，在**同一个列表**里同时提供 Codex 与 Claude 的可见聊天模型；每个选项必须携带自己的 `provider`，切换时不得继续沿用卡片切换前的 provider。
- R20: 手机端详情页滚动到任意历史位置时，**返回列表**必须始终可见；返回入口应放在吸顶页头中，不得要求用户先滚回会话顶部。
- R21: 混合模型列表仍需过滤 Git、音乐、文件树等工具模型，并继续由共享模型目录提供顺序与标签，避免手机端维护第二份模型清单。
