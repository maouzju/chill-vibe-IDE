...
1. **`App.tsx`**:
   ...
   - `failed` is sticky, but it is **scoped to the stream that died**. `markRecoveryFailed` tags it with that `streamId`, and `attachStream` calls `shouldClearRecoveryStatusForNewStream` before opening any stream: a different `streamId` retires the banner, the same one keeps it. This is what stops a card from showing `重连失败` while it is visibly working again — the Claude keepalive pool does not kill the CLI on a `result.is_error` turn, so the pooled process can finish its background work and push an **unsolicited** turn (`subscribeUnsolicitedStreams` in `App.tsx`) that puts the card back into `streaming` without going through `sendMessage`/manual resume. Putting the check in `attachStream` covers every present and future re-stream entrypoint instead of one more per-call-site reset.
   ...
   - Apply the native completion fact-check to Codex as well as Claude. `server/native-turn-completion.ts` reads the latest root `task_started` / `task_complete` marker from the matching rollout; `completed` finalizes locally, while `incomplete` / `unknown` remain fail-open. Both the HTTP route and packaged Electron `getNativeTurnCompletion` bridge must route Codex requests; a Claude-only bridge guard silently turns every packaged Codex result into `unknown`.
   - Drive the Codex stall timer through `resolveLocalStreamStallTimeoutMs()` for normal silence, in-progress commands, and active sub-agents. All open-work states are bounded by the absolute hard cap. On expiry, consult the native rollout before emitting a recoverable error so a lost terminal notification does not trigger a ghost continuation.
