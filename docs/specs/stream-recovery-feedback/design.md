...
1. **`App.tsx`**:
   ...
   - Apply the native completion fact-check to Codex as well as Claude. `server/native-turn-completion.ts` reads the latest root `task_started` / `task_complete` marker from the matching rollout; `completed` finalizes locally, while `incomplete` / `unknown` remain fail-open.
