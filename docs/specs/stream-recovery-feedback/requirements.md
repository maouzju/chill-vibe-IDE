...
- **#109** - Codex native reconnect placeholders can arrive on stderr, not only JSON-RPC stdout events.
...
9. **Stats reflect local reconnects** - When Codex emits native `Reconnecting... n/5` placeholders before a recoverable local-stream retry, whether as JSON-RPC stdout events or stderr diagnostics, Settings -> Routing -> Auto-retry stats must count a disconnect immediately, and automated recovery attempts must not inflate the request count.
10. **Native placeholders stay out of chat text** - Codex native `Reconnecting... n/5` placeholder deltas, completed assistant-message items, or stderr diagnostics are control signals only; they must not be persisted or rendered as normal assistant content/error text.
