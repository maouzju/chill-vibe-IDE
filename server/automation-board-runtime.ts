...
const workspaceAdminInstructionZh =
  ...
  + 'set_session_wake_timer 给某个会话挂计划唤醒（mode=duration 就是"过 N 分钟再回来看"；传 cardIds 就是"精确等这几个会话结束"，用户自己的聊天窗口和无关会话都压不住它，timeoutMinutes 是兜底上限）；'
...
const workspaceAdminInstructionEn =
  ...
  + 'set_session_wake_timer arms a wake timer on a session (mode=duration means "check back after N minutes"; pass cardIds to wait for exactly those sessions so the user\'s own chat windows cannot hold it back, timeoutMinutes being the upper bound); '
