...
const recoverableErrorPatterns = [
  ...
  // 症状：短暂的上游/代理抖动会直接把卡片红成 `API Error: Unable to connect
  //   to API (ConnectionRefused)`，用户只能手点续传。
  // 根因：2026-08-23 现场：CLI 对建连失败使用这一组固定文案，既不包含
  //   `closed before completion`，也不包含 `socket connection was closed`，所以
  //   现有断线名单完全命不中。它与上述瞬时 socket 断连同属可恢复网络故障。
  // 被否决：把所有 `fetch failed` / `timeout` 都放进名单会把代理配置、DNS 和
  //   认证类永久错误拖进重试；这里只接 CLI 明确的 connection-refused 词形，
  //   仍由 live session + renderer retry budget 两道护栏限流。
