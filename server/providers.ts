...
const getClaudeAskUserQuestionInstruction = (language: AppLanguage) =>
  normalizeLanguage(language) === 'en'
    ? 'In this Chill Vibe Claude runtime, the <ask-user-question> XML block is a rendering convention used ONLY for asking the user a question — it renders as an interactive choice card. It is not a substitute for tools: every real action (running commands, reading or editing files, searching, etc.) must still go through normal native tool calls, never written out as text. When, and only when, you must ask the user to choose before you can continue safely, reply with only one XML block in this exact shape and no extra text: <ask-user-question>{"header":"Short title","question":"One concise question","multiSelect":false,"options":[{"label":"Option A","description":"Short tradeoff"},{"label":"Option B","description":"Short tradeoff"}]}</ask-user-question>. Use 2-3 options, keep labels short, omit any Other option, and wait for the next user reply after emitting the block.'
    : '在这个 Chill Vibe 的 Claude 运行环境里，<ask-user-question> XML 块是一种渲染约定，仅用于向用户提问 —— 它会被渲染成可交互的选择卡片。它不是工具的替代品：所有实际操作（运行命令、读写文件、搜索等）都必须照常走原生工具调用，绝不能写成文本。仅当你必须在继续之前让用户做选择时，才只输出一个完整的 XML 块，并且不要加任何其他文本：<ask-user-question>{"header":"简短标题","question":"一句简洁问题","multiSelect":false,"options":[{"label":"选项 A","description":"简短权衡"},{"label":"选项 B","description":"简短权衡"}]}</ask-user-question>。选项保持 2-3 个，label 要简短，不要自己加 Other，并在输出这个块后等待用户下一条回复。'
