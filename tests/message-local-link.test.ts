...
test('renders Windows backslash file destinations as local message links', () => {
  const targetPath = String.raw`D:\Git\example-wiki\example-wiki\docs\sample-note.md`
  const markup = renderToStaticMarkup(
    React.createElement(
      React.Fragment,
      null,
      renderMarkdown(
        `新文章已生成并写入：[sample-note.md](${targetPath})`,
        String.raw`D:\Git\example-wiki`,
      ),
    ),
  )
  ...
  assert.equal(
    ...
    'D:/Git/example-wiki/example-wiki/docs/sample-note.md',
  )
  assert.equal(
    resolveMessageLocalLinkTarget(renderedHref ?? '', String.raw`D:\Git\example-wiki`),
